...
int guidance_init() {
	...
	// # set guidance state
	gs.mode = GS_MODE_INIT;
	gs.mode_next = GS_MODE_INIT;
...
int guidance_main(plant_state_t* ps, guidance_state_t* gs) {
	gs->status = 0;
	gs->mode = gs->mode_next;
	switch (gs->mode) {
		case GS_MODE_INIT:
			gs->status = guidance_core(1, ps->pos_, ps->vel_, gs);
			if (0 > gs->status) {
				return gs->status;
			}
			gs->t = 0.0;
			gs->t_c = 0.0;
			gs->cyc_cnt = 0;
			gs->mode_next = GS_MODE_WAIT;
			gs->out_cmd = 1;
			break;
		case GS_MODE_UPDATE:
			gs->a_thrust = gp.f_thrust/gs->mass;
			gs->status = guidance_core(0, ps->pos_, ps->vel_, gs);
			if (0 > gs->status) {
				return gs->status;
			}
			gs->t_c = 0.0;
		...
		case GS_MODE_WAIT:
			gs->cyc_cnt++;
			gs->out_cmd = 1;
			if (gp.cyc_wait <= gs->cyc_cnt) {
				if (gp.t_go_min < (gs->t_go - 1.0)) {
					gs->mode_next = GS_MODE_UPDATE;
				}
				else {
					gs->mode_next = GS_MODE_FINAL;
				}
			}
			break;
		case GS_MODE_FINAL:
			if (gp.t_go_min < gs->t_c) {
				gs->mode_next = GS_MODE_OFF;
				gs->out_cmd = 0;
			}
			break;
		default:
	...
	if (gs->out_cmd) {
		...
		// # update mass
		gs->mass = gp.mass_0 + gp.mass_dot*gs->t;

		// # calc acceleration vector
		// ## Pfun=@(t)lambda_.*cos(lambda_dot.*(t-K))+lambda_dot_/lambda_dot.*sin(lambda_dot.*(t-K));% <ref1 p569 eq15>
		vec3_t _lmd_xxx_k_ = scl_v3((cos(gs->lmd_dot*(gs->t_c - gs->k_time))), gs->lmd_);
		vec3_t _lmd_dot_lmd_dot_ = scl_v3((1.0/gs->lmd_dot), gs->lmd_dot_);
		vec3_t _lmd_dot_xxx_k_ = scl_v3(sin(gs->lmd_dot*(gs->t_c - gs->k_time)), _lmd_dot_lmd_dot_);
		gs->p_ = add_v3_v3(_lmd_xxx_k_, _lmd_dot_xxx_k_);

		// ## F_T_=F_T*P_(t_c);% [N] thrust vector
		gs->f_thrust_ = scl_v3(gp.f_thrust, gs->p_);

		// ## a_T_=F_T_/m(t);% [m/s^2] acceleration due to thrust
		gs->a_thrust_ = scl_v3((1.0/gs->mass), gs->f_thrust_);

		// # update dlt_v_thrust
		// ## Deltav_T_=Deltav_T_+a_T_*dt_c;% accumulate change in speed due to thrust
		vec3_t _a_thrust_dt_ = scl_v3(gp.dt, gs->a_thrust_);
		gs->dlt_v_thrust_ = add_v3_v3(gs->dlt_v_thrust_, _a_thrust_dt_); 

		// # plant interface
		ps->acl_ = gs->a_thrust_;
	}
	else {
		ps->acl_ = zeros_v3();
	}
...
int guidance_core(int32_t init, vec3_t r_, vec3_t v_, guidance_state_t* gs) {
	...
	if (init) {
		...
		_i_corr_max = gp.i_corr_max_init;
		_skip = 1;
		...
		_a_thrust = gs->a_thrust;
		_dlt_v_s_ = gs->dlt_v_thrust_;
	}
	else {
		// ## initialize for subsequent calls after first call
		_i_corr_max = gp.i_corr_min;
		_skip = 0;
	...
	for (i_corr = 0; i_corr <= _i_corr_max; i_corr++) {
		...
		if (!_skip) {
			...
			_tau = gp.v_ex/_a_thrust;

			// ## T_B = tau*(1-exp(-L/V_ex)) [s] burn time <ref1 p571>
			_t_b = _tau*(1.0 - exp(-1.0*(_l/gp.v_ex)));
			...
			// # predictor <ref1 p577>
			for (i_pred = 0; i_pred < gp.i_pred_max; i_pred++) {
				...
				if(gp.eps_r >= _abs_norm_r_p_r_p) {
					break;
				}
			}
			if (gp.i_pred_max <= i_pred) {
		...
		_u_x_sin_gmm_d_ = scl_v3(sin(gp.gmm_d), _u_x_);
		_u_z_cos_gmm_d_ = scl_v3(cos(gp.gmm_d), _u_z_);
		_u_x_sin_gmm_d_u_z_cos_gmm_d_ = add_v3_v3(_u_x_sin_gmm_d_, _u_z_cos_gmm_d_);
		_v_d_ = scl_v3(gp.v_d, _u_x_sin_gmm_d_u_z_cos_gmm_d_);
		...
		_s_eps = -gp.d_damp*(_u_y_.z + cos(gp.i_d))*_u_z_.z/_1_u_x_z_square;
		...
		_eps_v_d_v = gp.eps_v*_d_v;
		if ((gp.i_corr_min < i_corr) && (_eps_v_d_v >= _v_miss)) {
			break;
		}
	}
