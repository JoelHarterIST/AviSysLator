...
# ---
# version 2025.05.23 複数CSVファイル対応
# version 2025.06.04 DBL対応
# version 2025.07.02 複数シーケンス／コンポーネント対応
# ---
# 使い方
# 1. シーケンス制御機能で制御した機能のインタフェース公開用構造体を定義したヘッダファイルを用意する（例：fcu_if.h）。
#    注：状態やオプションの識別子はENUMを使用せず、defineで定義すること。
# 2. spreadsheet:seq_data_base_(nameseq)にシーケンス情報を入力る。
# 3. シーケンス情報を定義したシートをcsvで出力する。
# 4. 出力したファイルを本pythonスクリプトと同じディレクトリに配置する。
# 5. 本スクリプトを実行する。
# 6. 出力されたseq_data_base_(nameseq).hを適切な場所に配置する。
...
# input sequence name
nameseq = files[0].split(' ')[0].replace('seq_data_base_', '')
print(f'Sequence name: {nameseq}')
...
str_seq_id_data_base = f'int {O_SEQ_ID_DATA_BASE}_{nameseq}[{O_SEQ_ID_MAX_DEF}] = {{\n'
total_step_num = 0
str_seq_step_data_base = f'{O_EVENT_DATA_T}* {O_SEQ_STEP_DATA_BASE}_{nameseq}[{O_SEQ_STEP_MAX_DEF}] = {{\n'
total_event_num = 0
str_event = f'{O_EVENT_DATA_T} {O_EVENT_DATA_BASE}_{nameseq}[{O_EVENT_NUM_MAX}] = {{\n'
total_judge_num = 0
str_judge = f'{O_JUDGE_DATA_T} {O_JUDGE_DATA_BASE}_{nameseq}[{O_JUDGE_NUM_MAX}] = {{\n'
total_exec_num = 0
str_exec = f'{O_EXEC_DATA_T} {O_EXEC_DATA_BASE}_{nameseq}[{O_EXEC_NUM_MAX}] = {{\n'
...
for j in range(lenfiles):
	...
	for i in range(step_num):
		strtmp = strtmp + f'\t&{O_EVENT_DATA_BASE}_{nameseq}[{str_seq_event_base}+{dfSeqStep.loc[i,I_EVENT_ID]}],'
	...
	for i in range(event_num):
		...
		if type(dfSeqEvent.loc[i, I_NEXT_EVENT]) is str:
			str_next_event = f'({O_ADDRESS_T})&{O_EVENT_DATA_BASE}_{nameseq}[{dfSeqEvent.loc[i, I_NEXT_EVENT]}]'
		else:
			str_next_event = '0'
		if type(dfSeqEvent.loc[i, I_JUDGE_ID]) is str:
			str_event_judge = f'({O_ADDRESS_T})&{O_JUDGE_DATA_BASE}_{nameseq}[{str_seq_judge_base}+{dfSeqEvent.loc[i, I_JUDGE_ID]}]'
		else:
			str_event_judge = '0'
		if type(dfSeqEvent.loc[i, I_EXEC_ID]) is str:
			str_event_exec = f'({O_ADDRESS_T})&{O_EXEC_DATA_BASE}_{nameseq}[{str_seq_exec_base}+{dfSeqEvent.loc[i, I_EXEC_ID]}]'
		else:
			str_event_exec = '0'
		strtmp = strtmp \
	...
	for i in range(judge_num):
		if type(dfSeqJudge.loc[i, I_JUDGE_NEXT]) is str:
			str_next_judge = f'({O_ADDRESS_T})&{O_JUDGE_DATA_BASE}_{nameseq}[{dfSeqJudge.loc[i, I_JUDGE_NEXT]}]'
		else:
			str_next_judge = '0'
		if 'DBL' == dfSeqJudge.loc[i, I_JUDGE_TYPE]:
			val_thrs = float(dfSeqJudge.loc[i, I_JUDGE_THRS])
			byte_thrs = st.pack('>d', val_thrs)
			str_thrs = f'0x{byte_thrs.hex()}'
		else:
			str_thrs = dfSeqJudge.loc[i, I_JUDGE_THRS]
		strtmp = strtmp \
				...
				+ f', {str_next_judge}}},' \
				+ f'\t// SEQ {j} : {dfSeqJudge.loc[i, I_JUDGE_NAME]} {dfSeqJudge.loc[i, I_JUDGE_THRS]}' \
				+ '\n'
	...
	for i in range(exec_num):
		if type(dfSeqExec.loc[i, I_EXEC_NEXT]) is str:
			str_next_exec = f'({O_ADDRESS_T})&{O_EXEC_DATA_BASE}_{nameseq}[{dfSeqExec.loc[i, I_EXEC_NEXT]}]'
		else:
			str_next_exec = '0'
		if 'DBL' == dfSeqExec.loc[i, I_EXEC_TYPE]:
			val_src = float(dfSeqExec.loc[i, I_EXEC_SRC])
			byte_src = st.pack('>d', val_src)
			str_src = f'0x{byte_src.hex()}'
		else:
			str_src = dfSeqExec.loc[i, I_EXEC_SRC]
		strtmp = strtmp \
				+ f'\t{{{dfSeqExec.loc[i, I_EXEC_TYPE]}' \
				+ f', ({O_ADDRESS_T})&{dfSeqExec.loc[i, I_EXEC_DEST]}' \
				+ f', {str_src}' \
				+ f', {str_next_exec}}},' \
				+ f'\t// SEQ {j} : {dfSeqExec.loc[i, I_EXEC_NAME]} {dfSeqExec.loc[i, I_EXEC_SRC]}' \
				+ '\n'
...
# output
with open(f'seq_data_base_{nameseq}.h', mode='w') as f:
	...
	f.write('\n')
	f.write('// extern\n')
	f.write(f'extern int {O_SEQ_ID_DATA_BASE}_{nameseq}[{O_SEQ_ID_MAX_DEF}];\n')
	f.write(f'extern {O_EVENT_DATA_T}* {O_SEQ_STEP_DATA_BASE}_{nameseq}[{O_SEQ_STEP_MAX_DEF}];\n')
	f.write(f'extern {O_EVENT_DATA_T} {O_EVENT_DATA_BASE}_{nameseq}[{O_EVENT_NUM_MAX}];\n')
	f.write(f'extern {O_JUDGE_DATA_T} {O_JUDGE_DATA_BASE}_{nameseq}[{O_JUDGE_NUM_MAX}];\n')
	f.write(f'extern {O_EXEC_DATA_T} {O_EXEC_DATA_BASE}_{nameseq}[{O_EXEC_NUM_MAX}];\n')
	f.write('\n')
