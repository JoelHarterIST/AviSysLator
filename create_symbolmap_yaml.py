...
def read_symbolmap_dump(symbolmap_file_path:str, v_map:ValInfoMap, sim_mode:SimMode):
    """
    Description:
    ...
    for v in v_map._v_list:
        # print(v._name)
        df1 = df[df['name'].str.strip() == v._name]
        # print(df1)
        if (len(df1) > 0):
...
def create_symbolmap_yaml(header_file_list:list[str], obj_file:str, sybl_map_path:str, sim_mode:SimMode):
    """
    Description:
    ...
    if sim_mode == SimMode.SILS:
        write_symbolmap_dump(obj_file, sybl_temp_path)

    # symbolmapを読む
    v_map._v_list.append(ValInfo("mark_variable", "int32_t", 0))
    v_map = read_symbolmap_dump(sybl_temp_path, v_map, sim_mode)
...
if __name__ == '__main__':
    # パーサーを作成
    parser = argparse.ArgumentParser(description="create symbol map yaml file")

    # オプション引数を追加
    parser.add_argument("--obj_file", help="specify object file")
    parser.add_argument("--header_file_list", type=str, nargs="*", help="specify header file list")
    parser.add_argument("--sim_mode", choices=['SILS', 'HILS'], help="specify simulation mode")
    ...
    if args.sim_mode == 'HILS':
        sim_mode = SimMode.HILS
    else:
        sim_mode = SimMode.SILS    

    create_symbolmap_yaml(args.header_file_list, args.obj_file, sybl_map_path, sim_mode)
