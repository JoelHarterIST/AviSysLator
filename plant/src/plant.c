...
int eq_of_system(plant_params_t* pp, double* x, double* dx, double* u_) {
...
int plant_main(double* t, double dt, plant_state_t* ps) {
	...
	v3_to_array(ps->pos_, &_x_in[0]);
	v3_to_array(ps->vel_, &_x_in[3]);
	v3_to_array(ps->acl_, &_u_[0]);
	for (_step_rk4=0; _step_rk4 < 4; _step_rk4++) {
		...
		eq_of_system(&pp, _x_tmp, &_k[_step_rk4][0], &_u_[0]);
	}
	...
	*t = *t + dt;
	ps->pos_ = array_to_v3(&_x_out[0]);
	ps->vel_ = array_to_v3(&_x_out[3]);
